...
*/						if(trigger==1) {
				...
				$("#add-fixedassets").validate({
					...
					submitHandler: function (form) {
						...
						if(!$("#new_asset").is(':checked')) {
							...
							if(frequency==1) {
								var depnTill = $('#depreciation_date').val();
								var today    = $('#today').val();
								if(today!=depnTill) {
									alert('Please enter correct depreciation till date');
									return false;
								} else {
									form.submit();
									return false;
								}
								/*var split1   = depnTill.split('-');
								var split2   = today.split('-');
								var difference = split2[1]-split1[1];
								if((difference<=1 && (split1[2]==split2[2])) || (difference<0 && (split1[2]!=split2[2]) && split2[1]=='01')) {
									form.submit();
									return false;
								} else {
									alert('Please enter correct depreciation till date');
								...
								/*var split1   = depnTill.split('-');
								var split2   = fend.split('-');
								var difference = split2[2]-split1[2];
		    ...
		    $(document).on('click', '.approve_fixed_assets', function(e) {
				$("#add-fixedassets").validate({
					debug: false,
					rules: {
